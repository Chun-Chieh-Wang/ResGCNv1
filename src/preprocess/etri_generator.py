...
class NTU_Generator():
    def __init__(self, args, dataset_args):
        ...
        etri_ignored = '{}/ignore_etri.txt'.format(os.path.dirname(__file__))
        ...
        training_samples['etri-xview'] = []
        training_samples['ntu-xsub'] = []

        self.training_sample = training_samples[self.dataset]

        # Get ignore samples
        if os.path.exists(etri_ignored):
            with open(etri_ignored, 'r') as f:
                self.ignored_samples = [line.strip() + '.csv' for line in f.readlines()]
        else:
            logging.info('')
            logging.error('Error: Do NOT exist ignored sample file {}'.format(etri_ignored))
            raise ValueError()

        # Get skeleton file list
        self.file_list = []
        for folder in [dataset_args['etri_data_path']]:
    ...
    def read_skeleton_filter(self, file):
        with open(file, newline='') as f:
            ...
            csv=pd.read_csv(f)
            skeleton_sequence['numFrame'] = int(len(csv.frameNum))
